...
if ($kitset_or_service == true) {
    ...
    $th = array(_("Location"), _("Quantity On Hand"), _("Re-Order Level"),
        _("Demand"), _("Available"), _("On Order"), _("Batch report"));
}
table_header($th);
...
while ($myrow = db_fetch($loc_details)) {
    ...
    if ($kitset_or_service == false) {
        ...
        label_cell(viewer_link("Batch", '/inventory/inquiry/stock_status_per_batch.php?stock_id='.$_POST['stock_id'].'&loc_code='.$myrow["loc_code"]));
        end_row();
    } else {
