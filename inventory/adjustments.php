...
function check_batch($id,$qty){
    $stock_id = $id;
    if(!isset($_SESSION["batch"][$stock_id])){
        return false;
    }
    $entered_qty = $_SESSION["batch"][$stock_id]["entered_qty"];
    $batch_number = $_SESSION["batch"][$stock_id]["batch_number"];

    if(array_sum($entered_qty)!=$qty){
        return false;
    }
    return true;
}
...
if (isset($_POST['Process']) && can_process()) {
    ...
    $trans_no = add_stock_adjustment($_SESSION['adj_items']->line_items, $_POST['StockLocation'], $_POST['AdjDate'], $_POST['ref'], $_POST['memo_'],$_SESSION["batch"]);
...
function handle_update_item() {
    $id = $_POST['LineNo'];
    $stock_id = $_SESSION['adj_items']->line_items[$id]->stock_id;
    if(!check_batch($stock_id, input_num('qty'))){
        display_error(_("The number of batch selected doesn't match with the overall number. Please adjust the number again"));
        return false;
    }
...
function handle_new_item() {
    if(!check_batch($_POST['stock_id'], input_num('qty'))){
        display_error(_("The number of batch selected doesn't match with the overall number. Please adjust the number again"));
        return false;
    }
