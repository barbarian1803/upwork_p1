...
function display_po_receive_items() {
    ...
    $th = array(_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),
        _("Outstanding"), _("This Delivery"), _("Batch Number"), _("Price"), _("Total"));
    table_header($th);
...
function can_process() {
    ...
    foreach ($_SESSION['PO']->line_items as $order_line) {
        if (is_stock_batch_controlled($order_line->stock_id)&&$_POST[$order_line->stock_id."batch_number"]=="") {
            display_error(_("Batch number is required for item ".$order_line->item_description));
            return false;
        }
    }
...
if (isset($_GET['PONumber']) && $_GET['PONumber'] > 0 && !isset($_POST['Update'])) {
    ...
    $_SESSION['PO']->reference = $Refs->get_next(ST_SUPPRECEIVE, array('date' => Today(), 'supplier' => $_SESSION['PO']->supplier_id));
    copy_from_cart();
}
...
if (isset($_POST['Update']) || isset($_POST['ProcessGoodsReceived'])) {
    ...
    foreach ($_SESSION['PO']->line_items as $line) {
        if (($line->quantity - $line->qty_received) > 0) {
            ...
            if (isset($_POST[$line->stock_id . "batch_number"]) && strlen($_POST[$line->stock_id . "batch_number"]) > 0) {
                $_SESSION['PO']->line_items[$line->line_no]->batch_number = $_POST[$line->stock_id . "batch_number"];
            }
        }
    }
